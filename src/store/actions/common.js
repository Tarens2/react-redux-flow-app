...
export const fetchPhones = () => dispatch => {
  return axios.get('/api/phones.json')
    .then(
    ...
    .then(
      json => {
        ...
        const orderPhones = localStorage.getItem('info') ? JSON.parse(localStorage.getItem('info')).order : [];
        dispatch(setOrder(orderPhones))
        const wishListPhones = localStorage.getItem('info') ? JSON.parse(localStorage.getItem('info')).wishList : [];
...
export const submitOrder = (submitData) => dispatch => {
  //написал бы post
  return axios.get('/api/order.json', submitData)
    .then(
      response => response.data,
      error => console.error(error)
    )
