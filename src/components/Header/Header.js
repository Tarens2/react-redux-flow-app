...
class Header extends React.Component {
  ...
  generateMenuItems = (wishList) => {
    return (wishList.map(phone => {
      return (<MenuItem style={{height: 'auto'}} key={phone.id}
                        onClick={this.openImage(phone.image)}>
        <ListItemIcon>
          <div style={{
            width: 60,
            height: 60,
  ...
  render() {
    const {wishListCount = 0, orderCount, orderPrice, wishList = []} = this.props;
      ...
      <AppBar position="static">
        <Toolbar>
          ...
          >
            ...
            {wishList.length ?
              <MenuItem to="/wish-list" component={Link}
                        onClick={this.handleClose}>Показать все</MenuItem> :
              <MenuItem onClick={this.handleClose}>Список пуст</MenuItem>
            }
          </Menu>
          <Button variant="raised" color="secondary" component={Link}
                  to="/order">
